...
# ==================================================
# 🔧 System Requirements Check (MODIFIED: Root check disabled)
# ==================================================
check_requirements() {
...
# ==================================================
# 🧩 Hyprland and Core Packages Installation (MODIFIED: Added pulseaudio removal)
# ==================================================
install_hyprland() {
  if [ "$IS_ARCH" -eq 1 ]; then
    log "🧩 Installing Hyprland and core packages from official repositories..."

    # ⚠️ FIX: Resolve conflict between pulseaudio and pipewire-pulse
    if pacman -Qq pulseaudio > /dev/null 2>&1; then
      log "🔄 Removing conflicting package: pulseaudio..."
      # -Rns: Remove, don't save config, remove unneeded dependencies
    ...
    # Attempt to remove pulseaudio on Debian-based systems as well
    sudo apt remove -y pulseaudio pulseaudio-module-bluetooth || true
    sudo apt install -y \
...
# ==================================================
# 📁 Configuration Deployment (MODIFIED: Full config deployment)
# ==================================================
deploy_configs() {
  ...
  # Deploy terminal configs (Kitty, Ghostty, Alacritty)
  log "🖥️ Deploying terminal configurations (Kitty, Ghostty, Alacritty)..."
...
configure_services() {
  ...
  if [ "$IS_ARCH" -eq 1 ]; then
    ...
    sudo apt update && sudo apt install -y network-manager pipewire wireplumber
    sudo systemctl enable NetworkManager
    sudo systemctl start NetworkManager
    systemctl --user enable pipewire pipewire-pulse wireplumber
...
show_summary() {
  ...
  echo "    ~/.config/alacritty/ (Alacritty terminal)"
